 ...
 */
public class PropertiesFilterLexerCustom extends DelegateLexer implements IdTableBuilding.ScanWordProcessor {

    /* From PropertiesFilterLexer */
    @Override
    public void advance() {
    ...
    /* Modified from IdTableBuilding */
    public static void scanWords(final IdTableBuilding.ScanWordProcessor processor,
        ...
        while (true) {
            while (true) {
                ...
                if ((c >= 'a' && c <= 'z') ||
                        (c >= 'A' && c <= 'Z') ||
                        (c >= '0' && c <= '9') ||
                        (Character.isJavaIdentifierStart(c) && c != '$' && c != '_')) {
                    break;
                }
                index++;
                if (mayHaveEscapes && c == '\\') {
                    index++; //the next symbol is for escaping
                }
            }
            ...
            if (index - index1 > 100) {
                continue; // Strange limit but we should have some!
            }
    ...
    protected final void scanWordsInToken(final int occurrenceMask, boolean mayHaveFileRefs, final boolean mayHaveEscapes) {
        ...
        scanWords(this, myCachedBufferSequence, myCachedArraySequence, start, end, mayHaveEscapes);
    ...
    public PropertiesFilterLexerCustom(final Lexer originalLexer, final OccurrenceConsumer occurrenceConsumer) {
        super(originalLexer);
